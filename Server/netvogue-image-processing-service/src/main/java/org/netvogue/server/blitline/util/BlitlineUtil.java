...
public class BlitlineUtil {
	...
	public static void sendBlitlineRequest(String srcURL, String uniqueId,
			ImageType imageType, String functionName) throws Exception {
...
//		return response;
	...
	private static SaveParameters createSaveParameters(String uniqueId,
			String sizeName) {
	...
	private static String mapObjectToJSON(Object object) {
		...
		try {
			...
			System.out.println(rfe.getMessage()
					+ "Error while creating JSON object");
		}
		return dataToSend;
	}

	private static BlitlineResponse performPostRequest(String postData) throws Exception {

		HttpPost postRequest = new HttpPost(
				ResourceLoader.getProperty("blitline_endpoint"));
		...
		return convertResponseData(responseString, BlitlineResponse.class);

	}

	private static BlitlineResponse convertResponseData(String response,
			Object responseClass) throws JsonParseException,
			JsonMappingException, IOException {
		return mapper.readValue(response,
				(Class<BlitlineResponse>) responseClass);
	}

	/*
	 * public void testMain() throws Exception { BlitlineMessageRequest
	 * blitlineMessageRequest = new BlitlineMessageRequest(); //
	 * blitlineMessageRequest
	 * .setSrc("http://cdn.blitline.com/filters/boys.jpeg");
	 * blitlineMessageRequest
	 * .setPostback_url("http://netvogue.ap01.aws.af.cm/blitlinetest");
	 * BlitlineFunction function = new BlitlineFunction();
	 * function.setName("blur"); // Map<String, String> map =
	 * function.getParams(); // map.put("X", "200"); // map.put("Y", "100"); //
	 * map.put("height", "400"); // function.setParams(map); SaveParameters
	 * parameters = new SaveParameters();
	 * parameters.setImage_identifier("MY_TEST_IMAGE_BLUR");
	 * S3DestinationDetails details = new S3DestinationDetails();
	 * details.setKey(
	 * "pavan1/gallery/3a8eff61-0c97-487c-a94e-6ea844fsdfsdsd534-blur");
	 * parameters.setS3_destination(details); function.setSave(parameters);
	 * List<BlitlineFunction> list = new ArrayList<BlitlineFunction>();
	 * list.add(function); blitlineMessageRequest.setFunctions(list); //
	 * BlitlineRequest blitlineRequest = new BlitlineRequest();
	 * blitlineRequest.setJson(blitlineMessageRequest); ObjectMapper mapper =
	 * new ObjectMapper(); String dataToPost =
	 * mapper.writeValueAsString(blitlineRequest);
	 * System.out.println(dataToPost); performPostRequest(dataToPost); }
	 */
