...
public class NetworkServiceImpl implements NetworkService {
  ...
  public Iterable<Network> getNetworks(String username, boolean onlyconfirmed, int pagenumber) {
    if (onlyconfirmed) {
      return networkRepo.getConfirmedNetworks(username, Constants.NETWORKPAGE_LIMIT * pagenumber,
          Constants.NETWORKPAGE_LIMIT);
    }
    return networkRepo.getNetworks(username, Constants.NETWORKPAGE_LIMIT * pagenumber, Constants.NETWORKPAGE_LIMIT);
  }
  ...
  @Override
  public User CreateNetwork(User userBy, String userTo, Notification notification, String error) {
    ...
    try {
      ...
      // networkRepo.CreateNetwork(userBy, userTo);
      otherUser = userRepo.findByusername(userTo);
      ...
      if (ResultStatus.SUCCESS == SaveNetwork(network, error)) {
        return otherUser;
      } else {
        return null;
      }
